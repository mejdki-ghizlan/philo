...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/31 19:27:20 by ael-mejd          #+#    #+#             */
/*   Updated: 2025/11/04 02:25:45 by ael-mejd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int init_data(t_table *table)
{
    ...
    while (i < table->num_of_philo)
    {
        table->philosopher[i].philo_id = i + 1; // maymkenxi nqolo philo raqm 0 kla kanbdaw men 1
        table->philosopher[i].eat_counter = 0;
        table->philosopher[i].left_fork = &table->forks[i];
        table->philosopher[i].right_fork = &table->forks[(i + 1) % table->num_of_philo];
